...
proc_microstructue <- function(base_wd, overwrite = FALSE) {
  ...
  micro_dir <- file.path(base_wd, 'untouchedRaw', 'microstructure')
  ...
  #get meal paradigms from file name
  micro_list[['paradigm']] <- as.numeric(sapply(micro_list[['filename']], function(x) substr(x, unlist(gregexpr('-', x))+1, unlist(gregexpr('.txt', x))-1), simplify = TRUE))

  #organize data into BIDS sourcedata/phenotype
  micro_list[['sourcedata_done']] <- sapply(micro_list[['paradigm']], function(x) util_copy_to_source(task_dir = micro_dir, task_str = 'micro', sub_id = x, ses_str = 'ses-1', overwrite = overwrite), simplify = TRUE)

  #process raw data
  micro_list[['rawproc_done']] <- sapply(micro_list[['sub_str']], function(x) util_task_micro(sub_str = x, ses_str = 'ses-1', bids_wd = file.path(base_wd, 'bids'), overwrite = overwrite), simplify = TRUE)
  ...
  if (file_pattern == 'PS1' | file_pattern == 'PS2' | file_pattern == 'PS3' | file_pattern == 'PS4'){
    ...
    micro_clean_labels['time_relative_hms'] <- 'time relative to start in hours:mins:seconds'
    ...
    micro_wide_labels['total_active_eating_c1'] <- 'coder 1 - total active eating time in minutes'
    micro_wide_labels['bite_latency_c1'] <- 'coder 1 - latency to first bite in minutes'
    micro_wide_labels['total_leave_chair_c1'] <- 'coder 1 - total time out of chair in minutes'
    micro_wide_labels['meal_duration_c1'] <- 'coder 1 - meal duration in minutes'
    ...
    micro_wide_labels['total_active_eating_c2'] <- 'coder 2 - total active eating time in minutes'
    micro_wide_labels['bite_latency_c2'] <- 'coder 2 - latency to first bite in minutes'
    micro_wide_labels['total_leave_chair_c2'] <- 'coder 2 - total time out of chair in minutes'
    micro_wide_labels['meal_duration_c2'] <- 'coder 2 - meal duration in minutes'
    ...
    micro_wide_event_labels['time_relative_hms_c1'] <- 'coder 1 - time relative to start in hours:mins:seconds'
    ...
    micro_wide_event_labels['time_relative_hms_c2'] <- 'coder 2 - time relative to start in hours:mins:seconds'
